...
import com.xzg.wlxx.lombok.annotation.designpattern.Singleton
...
internal class SingletonProcessor : SingleAnnotationProcessor() {
    ...
    inner class DCLTreeTranslator(private val rootClazzName: Name, private val constructor: Symbol.MethodSymbol?) :
        TreeTranslator() {
        ...
        private fun makeSyncDecl(jcClassDecl: JCClassDecl): JCTree.JCSynchronized? {
            return treeMaker.Synchronized(treeMaker.Parens(
                treeMaker.Select(
                    treeMaker.Ident(jcClassDecl.name),
                    names._class
                )
        ...
        private fun makeGetInstanceMethodDecl(jcClassDecl: JCClassDecl): JCMethodDecl {
            val body = ListBuffer<JCTree.JCStatement>()
                .append(
                    treeMaker.Return(
                        treeMaker.Select(
                            treeMaker.Ident(names.fromString("_InstanceHolder")),
                            names.fromString("_sInstance")
                        )
                    )
                )
